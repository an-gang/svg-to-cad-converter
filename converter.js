    ...
    function main() {
        ...
        $("polygon").each(function () {
            ...
            if (polygonMap.get(polygonName) === undefined) {
                ...
                if ($(this).parent().find("circle").length > polygonMap.get(polygonName)) {//2比1大
                    //保留第二个也就是当前循环
                    skip = false;
                } else if ($(this).parent().find("circle").length === polygonMap.get(polygonName)) {//如相等
            ...
            if (!skip) {
                ...
                //生成SHAPES部分
                shapesTemp += "INSERT SMD\n";
                shapesTemp += "SHAPE " + polygonName + "\n";
                $(this).parent().find("circle").each(function () {
                    if($(this).attr("number")!==""){
                        ...
                        shapesTemp += "PIN " + $(this).attr("number") + "  X " + -offsetY.toFixed(2) + " " + offsetX.toFixed(2) + " TOP 0.00 0\n";
                        //记录SIGNALS部分
                        var str = polygonName + " " + $(this).attr("number");
                        if (signalMap.get($(this).attr("net")) === undefined) {
                            signalMap.set($(this).attr("net"), [str]);
                        } else {
                            if (!signalMap.get($(this).attr("net")).includes(str)) {
                                signalMap.get($(this).attr("net")).push(str);
                            }
                        }
                    }
                });
                //生成COMPONENTS部分
                componentsTemp += "COMPONENT " + polygonName + "\n";
                componentsTemp += "PLACE " + -centerY.toFixed(2) + " " + centerX.toFixed(2) + "\n";//y轴坐标取反修复翻转
                if ($(this).parent().attr("class").indexOf("TOP") !== -1) {
                    componentsTemp += "LAYER TOP\n";
                } else {
                    componentsTemp += "LAYER BOTTOM\n";
                }
                componentsTemp += "ROTATION 0\n";
                componentsTemp += "SHAPE " + polygonName + " 0 0\n";
                componentsTemp += "DEVICE Device" + deviceIndex + "\n";
                //生成DEVICES部分
                ...
                devicesTemp += "DEVICE Device" + deviceIndex + "\n";
                devicesTemp += "PART " + bom + "\n";
                deviceIndex++;
                polygonMap.set(polygonName, $(this).parent().find("circle").length);
            }
        });
        //保存最后一次循环的结果
        if (!skip) {
